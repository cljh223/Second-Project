	...
	<script type="text/javascript">
		...
		function addMarker(lonlat) {
			...
			var icon = new Tmap.IconHtml(
					'<link rel="stylesheet" href="css/buttonstyle.css"><div class="radial-menu">'
		...
		// 팝업창 공통 구성을 만드는 메소드
		function popupString(){
			var popString = '<form><div class="row">';
			popString += '<div class="col-lg-6">';
			popString += '<div class="dataTables_filter" id="editable-sample_filter">';
			popString += '<label>Search: <input type="text" id = "searchText"';
			popString += 'aria-controls="editable-sample"';
			popString += 'class="form-control medium"></label>';
			popString += '</div>';
			popString += '</div>';
			popString += '</div><div class="col-sm-12">';
			popString += '<section class="panel">';
			popString += '<div class="panel-body">';
			popString += '<table id = "searchTable" class="table  table-hover general-table">';
			
			return popString;
		}
		
		// 팝업창을 띄우는 메소드
		function bootPopup(content){
			bootpopup({
		...
		// 직원 테이블을 만드는 메소드
		function shopStringFunction(resp){
			var shopString = '';
			for(var i = 0; i<resp.length; i++){
				shopString += '<tr id="'+resp[i].shopName+'">';
				shopString += '<td><a href="#">'
				shopString += resp[i].shopCode;
				shopString += '</a></td>'
				shopString += '<td class="hidden-phone">'
				shopString += resp[i].shopName;
				shopString += '</td>'
				shopString += '<td>';
				shopString += resp[i].shopRep;
				shopString += '</td>';
				shopString += '<td>';
				shopString += resp[i].shopTel;
				shopString += '</td>';
				shopString += '<td>';
				shopString += '<div class="progress progress-striped progress-xs">';
				shopString += '<div style="width: 40%" aria-valuemax="100" aria-valuemin="0" aria-valuenow="40" role="progressbar" class="progress-bar progress-bar-success">';
				shopString += '<span class="sr-only">40% Complete (success)</span>';
				shopString += '</div>' + '</div>' + '</td>';
				shopString += '</tr>';
			}
			shopString += '</tbody>' + '</thead>';
			shopString += '</div>' + '</section>';
			shopString += '</div></form>' ;
		...
		// 디비에서 매장 정보를 가져오는 메소드
		function shopBootFunction(){
			
			$.ajax({
				url : 'popupShop'
				, method : 'POST'
				, dataType : 'json'
				, success : shopBootString
				,error : function(){
					alert('에러입니다.');
				}
			});
		...
		// 매장테이블 전체를 팝업창에 띄우는 메소드. 검색창에 입력할 때 마다 테이블의 내용을 불러온다.
		function shopBootString(resp){
			bootPopup(
					popupString() 
					+ '<thead>'
					+ '<tr>'
					+ '<th>매장코드</th>'
					+ '<th class="hidden-phone">매장명</th>'
					+ '<th>대표자명</th>'
					+ '<th>전화번호</th>'
					+ '<th>생산률</th>'
					+ '</tr>'
					+ '</thead>'
					+ '<tbody id = "shopTable">'
					+ shopStringFunction(resp));
			$('#searchText').on('keyup', function(){
				var shopName = $('#searchText').val();
				var searchText = {'searchText' : shopName}
				$.ajax({
					url : 'popupShop'
					, method : 'get'
					, data : searchText
					...
					, success : function(respp){
						var searchResult = shopStringFunction(respp);
						$('#shopTable').html(searchResult);
						trShopFunction();
					}
					, error : function(){
						alert('에러입니다.');
		...
		function warehouseBootString(resp){
			...
			$('#searchText').on('keyup', function(){
				var warehouseName = $('#searchText').val();
				var searchText = {'searchText' : warehouseName}
				$.ajax({
					url : 'popupWarehouse'
					, method : 'get'
					, data : searchText
		...
		function staffBootString(resp){
			...
			$('#searchText').on('keyup', function(){
				var staffName = $('#searchText').val();
				var searchText = {'searchText' : staffName}
				$.ajax({
					url : 'popupStaff'
					, method : 'get'
					, data : searchText
		...
		function productBootString(resp){
			...
			$('#searchText').on('keyup', function(){
				...
				var searchText = {'searchText' : productName}
				$.ajax({
					url : 'popupProduct'
					, method : 'get'
					, data : searchText
		...
		function trShopFunction(){
			$('tr').on('click', function(){
				var shopName = $(this).attr('id');
				$('.btn-primary').on('click', function(){
					$('#firstname').val(shopName);
				});
			});
		}
		...
		function trProductFunction(resp){
			$('tr').on('click', function(){
				...
				$('.btn-primary').on('click', function(){
					...
					supplyTableString += resp[index].productCode;
					supplyTableString += '</td>'
					supplyTableString += '<td class="hidden-phone">'
					supplyTableString += resp[i].productName;
					supplyTableString += '</td>'
					supplyTableString += resp[i].productUnit;
					supplyTableString += '</td>';
					supplyTableString += '<td>'
					supplyTableString += '??';
					supplyTableString += '</td>';
					...
					supplyTableString += '</td>';
					supplyTableString += '<td>'
					supplyTableString += resp[i].productReleasePrice;
					supplyTableString += '</td>';
					supplyTableString += '<td>'
					supplyTableString += resp[i].productUnitPrice;
					supplyTableString += '</td>';
					$('#supplyProductTable').html(supplyTableString);
				});
			});
		}
		...
		$(function() {
			initialize();
			$('#firstname').on('click',shopBootFunction);
			$('#lastname').on('click',staffBootFunction);
			$('#username').on('click',warehouseBootFunction);
...
<body>
	<section id="container">
		...
		<section id="main-content">
			...
			<form class="cmxform form-horizontal" id="supplyForm" method="GET" action="">
				<div class="row">
					<div class="col-sm-6">
						<section class="panel">
							<header class="panel-heading">
								Advanced Form validations <span class="tools pull-right">
									...
									class="fa fa-cog" href="javascript:;"></a> <a
                    ...
                    <div class="panel-body">
	                    <div class="dataTables_filter" id="editable-sample_filter">
							<button type="button" id ="productSelectbutton" class="btn btn-info "><i class="fa fa-refresh"></i> 상품검색</button>
						</div>
